...
export default function PulpPagination({items, currentPage, onPageChange}) {
    ...
    return(
        <article>
            ...
            <StyledList>
              <StyledDiv>
                {currentPage != 1 && <StyledButton onClick={handleBackwards}><StyledBack /></StyledButton>}
                <p> Seite {currentPage} von 7</p>
                {currentPage != 7 && <StyledButton onClick={handleForwards}><StyledForward /></StyledButton>}
                </StyledDiv>
                <StyledDiv>
                {pages.map((page) => (
...
background: ${({$itemActive}) => ($itemActive ? "var(--dark-font)" : "var(--link)")};
display: flex;
 justify-content: center;
 align-items: center;
 ...
 color: transparent;
 &:hover {
 background: ${({$itemActive}) => ($itemActive ? "var(--dark-font)" : "var(--background)")};
 }
`;
